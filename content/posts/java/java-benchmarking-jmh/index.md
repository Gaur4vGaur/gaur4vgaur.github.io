...
Maven has already provided us with a template in `MyBenchmark` class to fill in. I am going to utilise the same class.
...
The next step is to execute the `jar` using the below command:
```shell
...
### Benchmark Modes
In the previous example, I used `@BenchmarkMode(Mode.AverageTime)`. If you try to decompile JMH jar, you will find `enum Mode` with the below values:
